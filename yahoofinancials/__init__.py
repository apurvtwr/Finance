...
class YahooFinanceETL(object):
    def __init__(self, ticker):
        self.ticker = (
            ticker.upper() if isinstance(ticker, str) else [t.upper() for t in ticker]
        )
        self._cache = {}
    ...
    def format_date(in_date):
        if isinstance(in_date, str):
            ...
            form_date = str(
                (
                    datetime.datetime(1970, 1, 1) + datetime.timedelta(seconds=in_date)
                ).date()
            )
        return form_date
            ...
            for i in range(0, max_retry):
                ...
                if response.getcode() != 200:
                    ...
                    if re_script is not None:
                        ...
                        self._cache[url] = loads(
                            re.search("root.App.main\s+=\s+(\{.*\})", script).group(1)
                        )
                        response.close()
                        break
                    else:
                ...
                if i == max_retry - 1:
                    ...
                    raise ManagedException(
                        "Server replied with HTTP "
                        + str(response.getcode())
                        + " code while opening the url: "
                        + str(url)
                    )
    ...
    def _format_time(self, in_time):
        form_date_time = datetime.datetime.fromtimestamp(int(in_time)).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def _build_historical_url(self, ticker, hist_oj):
        url = (
            self._BASE_YAHOO_URL
            + self._encode_ticker(ticker)
            + "/history?period1="
            + str(hist_oj["start"])
            + "&period2="
            + str(hist_oj["end"])
    ...
    def _clean_api_data(self, api_url):
        ...
        for result in results:
            ...
            ret_obj.update(
                {"firstTradeDate": result["meta"].get("firstTradeDate", "NA")}
            )
            ret_obj.update({"currency": result["meta"].get("currency", "NA")})
            ret_obj.update(
                {"instrumentType": result["meta"].get("instrumentType", "NA")}
            )
    ...
    def _create_dict_ent(
        self, up_ticker, statement_type, tech_type, report_name, hist_obj
    ):
        YAHOO_URL = (
            self._BASE_YAHOO_URL
            + up_ticker
            + "/"
            + self.YAHOO_FINANCIAL_TYPES[statement_type][0]
            + "?p="
            + up_ticker
        )
        if tech_type == "" and statement_type != "history":
            try:
                ...
                dict_ent = {
                    up_ticker: re_data[u"" + report_name],
    ...
    def _get_sub_dict_ent(self, ticker, raw_data, statement_type):
        form_data_list = self._reformat_stmt_data_process(
            raw_data[ticker], statement_type
        )
    ...
    def get_stock_data(
        self, statement_type="income", tech_type="", report_name="", hist_obj={}
    ):
        data = {}
        if isinstance(self.ticker, str):
            dict_ent = self._create_dict_ent(
                self.ticker, statement_type, tech_type, report_name, hist_obj
            )
            data.update(dict_ent)
        else:
            for tick in self.ticker:
                try:
                    dict_ent = self._create_dict_ent(
                        tick, statement_type, tech_type, report_name, hist_obj
                    )
                    data.update(dict_ent)
    ...
    def get_clean_data(self, raw_report_data, report_type):
        ...
        if isinstance(self.ticker, str):
            if report_type == "earnings":
                try:
                    cleaned_data = self._clean_earnings_data(
                        raw_report_data[self.ticker]
                    )
                except:
                    cleaned_data = None
            else:
    ...
    def _handle_api_dividend_request(self, cur_ticker, start, end, interval):
        ...
        div_dict = self._get_api_data(test_url)["chart"]["result"][0]["events"][
            "dividends"
        ]
    ...
    def get_stock_dividend_data(self, start, end, interval):
        ...
        if isinstance(self.ticker, str):
            try:
                return {
                    self.ticker: self._handle_api_dividend_request(
                        self.ticker, start, end, interval_code
                    )
                }
            except:
            ...
            for tick in self.ticker:
                try:
                    div_data = self._handle_api_dividend_request(
                        tick, start, end, interval_code
                    )
...
class YahooFinancials(YahooFinanceETL):
    ...
    def get_key_statistics_data(self, reformat=True):
        if reformat:
            return self.get_clean_data(
                self.get_stock_tech_data("defaultKeyStatistics"), "defaultKeyStatistics"
            )
        else:
    ...
    def get_summary_data(self, reformat=True):
        if reformat:
            return self.get_clean_data(
                self.get_stock_tech_data("summaryDetail"), "summaryDetail"
            )
        else:
        ...
        if isinstance(self.ticker, str):
            ...
            for tick in self.ticker:
                if self.get_stock_price_data()[tick] is None:
                    ...
                    ret_obj.update(
                        {tick: self.get_stock_price_data()[tick].get(data_field, None)}
                    )
            return ret_obj
        ...
        if isinstance(self.ticker, str):
            ...
            for tick in self.ticker:
                if self.get_summary_data()[tick] is None:
                    ...
                    ret_obj.update(
                        {tick: self.get_summary_data()[tick].get(data_field, None)}
                    )
            return ret_obj
    ...
    def get_book_value(self):
        return self._financial_statement_data(
            "balance",
            "balanceSheetHistoryQuarterly",
    ...
    def get_net_income(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "netIncome", "annual"
        )

    def get_interest_expense(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "interestExpense", "annual"
        )

    def get_operating_income(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "operatingIncome", "annual"
        )

    def get_total_operating_expense(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "totalOperatingExpenses", "annual"
        )

    def get_total_revenue(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "totalRevenue", "annual"
        )

    def get_cost_of_revenue(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "costOfRevenue", "annual"
        )

    def get_income_before_tax(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "incomeBeforeTax", "annual"
        )

    def get_income_tax_expense(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "incomeTaxExpense", "annual"
        )

    def get_gross_profit(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "grossProfit", "annual"
        )

    def get_net_income_from_continuing_ops(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "netIncomeFromContinuingOps", "annual"
        )

    def get_research_and_development(self):
        return self._financial_statement_data(
            "income", "incomeStatementHistory", "researchDevelopment", "annual"
        )
